...
`Get-AzPolicyImpactReport` executed locally or from the Azure Cloud Shell generates comprehensive reports analyzing Azure Policy assignments that could impact your resource deployments. It retrieves complete policy definitions including PolicyRule, PolicyParameters, and PolicyDefinitions across all Azure hierarchy levels: Management Group, Subscription, Resource Group, and individual Resource scopes. The report identifies which policies will block deployments versus those that only audit, analyzes your permission context to detect potential blind spots, and exports all data in JSON format for deployment planning and exemption requests. This enables you to understand Azure Policy requirements **before** attempting deployment, eliminating trial-and-error approaches to policy compliance.
...
3. **Run the function**
   ...
   **Specify a resource group directly:**
   ...
   ```
   > Automatically searches for the resource group across all accessible subscriptions. If found in one subscription, uses it automatically. If found in multiple subscriptions, prompts to select which one. Analyzes all policies that apply to the specified resource group from Management Group, Subscription, Resource Group, and individual Resource scopes.
...
- **Console Output:** Real-time policy collection status, permission analysis, scope summary showing Management Group/Subscription/Resource Group/Resource policy counts, and blind spot warnings
- **JSON Report:** Generated by default. Includes:
  ...
  - **ScopeAnalysis:** Summary statistics showing policy counts by scope (MG/Sub/RG/Resource)
...
- **Azure Reader Role:** An account with Reader or equivalent permissions. Minimum:
  - **Subscription Reader** - Required to collect subscription, resource group, and individual resource level policies
...
```
> Analyzes all policies at Management Group and Subscription scopes without targeting a specific resource group or resources. Useful for understanding organization-wide policy landscape.
