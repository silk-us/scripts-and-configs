...
Write-Host "Retrieving IAM role assignments and PIM eligibility for the current user..." -ForegroundColor Cyan

$roleDefCache = @{}
function Get-RoleDefRest {
    param([string]$Guid)
    if (-not $Guid) { return $null }
    if ($roleDefCache.ContainsKey($Guid)) { return $roleDefCache[$Guid] }
    $props = $null
    try {
        $path = "/subscriptions/$($azContext.Subscription.Id)/providers/Microsoft.Authorization/roleDefinitions/$($Guid)?api-version=2022-04-01"
        $resp = Invoke-AzRestMethod -Method GET -Path $path
        if ($resp.StatusCode -eq 200 -and $resp.Content) {
            $props = ($resp.Content | ConvertFrom-Json).properties
        }
    } catch { }
    $roleDefCache[$Guid] = $props
    return $props
}

# build one role object with its actions; used for both active and PIM roles.

function New-RoleObject {
    param([string]$RoleName, [string]$Scope, [string]$RoleDefinitionId, [string]$AssignmentType, [hashtable]$Extra)
    $guid = if ($RoleDefinitionId) { ($RoleDefinitionId -split '/')[ -1 ] } else { $null }
    $props = Get-RoleDefRest -Guid $guid
    if (-not $RoleName -and $props) { $RoleName = $props.roleName }
    $o = New-Object PSObject
    ...
    $o | Add-Member -MemberType NoteProperty -Name "Role Definition Id" -Value $RoleDefinitionId
    $o | Add-Member -MemberType NoteProperty -Name "Description" -Value $props.description
    $o | Add-Member -MemberType NoteProperty -Name "Is Custom" -Value ($props.type -eq 'CustomRole')
    $o | Add-Member -MemberType NoteProperty -Name "Actions" -Value (@($props.permissions.actions))
    $o | Add-Member -MemberType NoteProperty -Name "Not Actions" -Value (@($props.permissions.notActions))
    $o | Add-Member -MemberType NoteProperty -Name "Data Actions" -Value (@($props.permissions.dataActions))
    $o | Add-Member -MemberType NoteProperty -Name "Not Data Actions" -Value (@($props.permissions.notDataActions))
    if ($Extra) { foreach ($k in $Extra.Keys) { $o | Add-Member -MemberType NoteProperty -Name $k -Value $Extra[$k] } }
    return $o
}

# Active role assignments for whoever is running this
$iamRoles = @()
try {
    ...
    if ($account.Type -eq 'User') {
        ...
        $roleAssignments = Get-AzRoleAssignment -SignInName $account.Id -ExpandPrincipalGroups -ErrorAction SilentlyContinue
    } else {
        # service principal / managed identity context
        $roleAssignments = Get-AzRoleAssignment -ServicePrincipalName $account.Id -ErrorAction SilentlyContinue
    }
    Write-Host "Processing $(@($roleAssignments).Count) active role assignment(s)..." -ForegroundColor Cyan
    foreach ($ra in $roleAssignments) {
        try {
            Write-Host "  Active role: $($ra.RoleDefinitionName) @ $($ra.Scope)" -ForegroundColor Gray
            $iamRoles += New-RoleObject -RoleName $ra.RoleDefinitionName -Scope $ra.Scope -RoleDefinitionId $ra.RoleDefinitionId -AssignmentType 'Active'
        } catch {
            Write-Host "    Error processing role $($ra.RoleDefinitionName): $($_.Exception.Message)" -ForegroundColor Red
        }
    }
} catch {
    Write-Host "Error retrieving IAM role assignments: $($_.Exception.Message)" -ForegroundColor Red
    $iamRoles = @()
}

# PIM eligible assignments for the current user (asTarget). Needs Entra ID P2;
# if PIM isn't enabled it just comes back empty.
$pimRoles = @()
try {
    $pimScope = "/subscriptions/$($azContext.Subscription.Id)"
    $pimEligible = Get-AzRoleEligibilityScheduleInstance -Scope $pimScope -Filter "asTarget()" -ErrorAction SilentlyContinue
    Write-Host "Processing $(@($pimEligible).Count) PIM eligible assignment(s)..." -ForegroundColor Cyan
    foreach ($pe in $pimEligible) {
        try {
            $extra = @{
                'Member Type' = $pe.MemberType
                'Status'      = $pe.Status
                'Start Time'  = $pe.StartDateTime
                'End Time'    = $pe.EndDateTime
            }
            Write-Host "  Eligible role @ $($pe.ScopeId)" -ForegroundColor Gray
            $pimRoles += New-RoleObject -Scope $pe.ScopeId -RoleDefinitionId $pe.RoleDefinitionId -AssignmentType 'Eligible (PIM)' -Extra $extra
        } catch {
            Write-Host "    Error processing PIM role: $($_.Exception.Message)" -ForegroundColor Red
        }
    }
} catch {
    Write-Host "PIM eligible assignments unavailable (PIM / Entra ID P2 may not be enabled): $($_.Exception.Message)" -ForegroundColor Yellow
    $pimRoles = @()
}

# Bundle the user's active + eligible roles and hang them off the cluster object
$userScopes = New-Object PSObject
$userScopes | Add-Member -MemberType NoteProperty -Name IAMRoles -Value $iamRoles
$userScopes | Add-Member -MemberType NoteProperty -Name PIMRoles -Value $pimRoles
